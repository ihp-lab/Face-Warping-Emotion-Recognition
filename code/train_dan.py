...
from adaptation.dan import MultipleKernelMaximumMeanDiscrepancy
...
def main():
	...
	# Names, paths, logs
	parser.add_argument('--logger_path', default='checkpoints/dan', help='relative path to log')
...
class Solver():
	def __init__(self, opt):
		...
		self.domain_criterion = MultipleKernelMaximumMeanDiscrepancy(
			kernels=[GaussianKernel(alpha=2 ** k) for k in range(-3, 2)],
			linear=not self.opt.non_linear, quadratic_program=self.opt.quadratic_program)
	...
	def train_model(self):
		...
		for epoch in range(self.opt.epochs_num):
			...
			label_loss, domain_loss, train_ccc = self.train()
	...
	def train(self):
		...
		return label_loss, domain_loss, train_ccc
